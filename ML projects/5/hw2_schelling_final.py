...
### using your knowledge of list methods, combine these into one list
...
testlist = [PurpleAgents(random.randint(0,100), random.randint(0,100)) for x in range(400)] + [GoldAgents(random.randint(0,100), random.randint(0,100)) for x in range(400)]
...
class PurpleDiversitySeekers(AgentNew):
    def __init__(self, xlocation, ylocation, group="Purple", status="unhappy"):
        super().__init__(xlocation, ylocation, group, status)
    def move_if_unhappy(self):  # what arguement(s) are needed here?
        if self.status=="happy":  ### there is a mistake in this command - fix it
            self.x = random.randint(0,100)
            self.y = random.randint(0,100)
...
group_affinity_threshold = .51
testlist = [PurpleAgents(random.randint(0,100), random.randint(0,100)) for x in range(300)] + [GoldAgents(random.randint(0,100), random.randint(0,100)) for x in range(300)] + [PurpleDiversitySeekers(random.randint(0,100), random.randint(0,100)) for x in range(100)] + [GoldDiversitySeekers(random.randint(0,100), random.randint(0,100)) for x in range(100)] 
###### create a testlist that has 300 PurpleAgents and 300 GoldAgents and 100 PurpleDiversitySeekers and 100 GoldDiversitySeekers
map_colorful_agents(testlist)
for x in range(15):
    for agent in (testlist):
        agent.check_neighbors(testlist)  #does this need any arguments?
